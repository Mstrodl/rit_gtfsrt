      ...
      if csv_route.route_id == trip.route_id {
        if let Some(frequency) = self.csv_frequencies.get(&trip.trip_id) {
          for stop_time in &self.csv_stop_times {
            ...
            println!(
              "Candidates: floor {} and ceil {}",
              day_time_serializer(floored_candidate as u64),
              day_time_serializer(ceiled_candidate as u64)
            );
            println!("Delta floor {delta_floor} delta ceil {delta_ceil} s");
            println!("Trip iteration was: {}", trip_iteration);
            let trip_iteration = if delta_ceil > delta_floor {
              trip_iteration.floor()
            } else {
              trip_iteration.ceil()
            } as u64;
            println!("Trip iteration is: {}", trip_iteration);
            println!();
